...
# cul2mqtt@<name> services there. Same script as in lgtv2mqtt / lgsb2mqtt.
#
# Usage:
...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/cul2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                cul2mqtt@<name> instance plus a plain cul2mqtt unit if present
...
# serialport 13 ships prebuilt binaries for linux arm/x64, so no toolchain is needed
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n cul -s /dev/ttyACM0 -u mqtt://broker)"
fi
