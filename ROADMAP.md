...
- [ ] Switch `package.json` from `file:../mqtt-interfaces-core` to `^0.2.0` and from `file:../cul`
      to `^1.0.0` once both are on npm (`deploy.sh` ships `file:` dependencies as tarballs until
      then), then release 1.0.0.
